...
public class MillenniumFalconService {
     ...
     */
    public float computeBestOddsToReachDestination(EmpireInformationDto empireInformationDto) {
        ...
        var accessibleRoutes = routeService.getAccessibleRoutesMap();

        val departureStep = new TravelInProcess()
                .setCurrentPosition(millenniumFalconConfiguration.getDeparture())
                .setAutonomy(millenniumFalconConfiguration.getAutonomy())
                .setNumberOfDays(0);
        ...
        while (!queue.isEmpty() && minimumNumberOfMeetingsWithHunters != 0) {
            ...
            if (travelInProcess.getNumberOfMeetingsWithHunters() < minimumNumberOfMeetingsWithHunters &&
                ...
                if (travelInProcess.getCurrentPosition().equals(millenniumFalconConfiguration.getArrival())) {
                ...
                refuelStep(travelInProcess, queue);
            }
        }
     ...
     */
    private void refuelStep(TravelInProcess travelInProcess, Queue<TravelInProcess> queue) {
